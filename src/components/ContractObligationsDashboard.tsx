...
export default function ContractObligationsDashboard({ projectId }: { projectId: string }) {
  ...
  return <div className="space-y-5 text-xs" key={projectId}>
    ...
    {showForm&&<form onSubmit={save} className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-slate-800/60 border border-slate-700 p-4 rounded-xl">
      ...
      <Input label="Obligation to Issue"><input required value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/></Input>
      <Input label="Responsible Party"><select required value={form.responsible_party} onChange={e=>setForm({...form,responsible_party:e.target.value})}><option value="">Choose party</option>{parties.map(party=><option key={party}>{party}</option>)}</select></Input>
