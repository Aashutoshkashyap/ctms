...
function buildReport(type:ReportType,p:Props){
  ...
  const head=`<h1>${project.name}</h1><p><b>${label(type)}</b> · Generated ${formatBsDate(today, { long: true })}</p><p>Contract ${project.contract_number||'—'} · ${project.currency} ${Number(project.contract_amount).toLocaleString()}</p>`;
  ...
  if(type==='daily'){rows=[['Date (BS)','Weather','Manpower','Equipment','Obstruction'],...p.dailyReports.map(r=>[formatBsDate(r.report_date),r.weather,r.manpower_total,r.equipment_total,r.obstruction_reasons])];section=table(rows);}
  if(type==='weekly'){const upcoming=p.activities.filter(a=>a.status!=='completed').slice(0,8);rows=[['BOQ Item','Description of Work','Status','Forecast Start (BS)','Forecast Finish (BS)','Float'],...upcoming.map(a=>[a.wbs_code,a.name,a.status,formatBsDate(a.early_start),formatBsDate(a.early_finish),a.total_float])];section=`<h2>Two-week Lookahead</h2>${table(rows)}<h2>Required Actions</h2><ul><li>Clear overdue design and procurement constraints.</li><li>Protect critical and near-critical work fronts.</li><li>Confirm labour, plant and materials for upcoming activities.</li></ul>`;}
  if(type==='monthly'){rows=[['BOQ Item','Description of Work','Status','Actual Qty','Planned Qty','Float'],...p.activities.map(a=>[a.wbs_code,a.name,a.status,a.actual_quantity,a.planned_quantity,a.total_float])];section=`${metrics}<h2>Progress and Schedule</h2>${table(rows)}<h2>Management Narrative</h2><p>The project is ${p.evm.spi>=1?'on/ahead of':'behind'} schedule and ${p.evm.cpi>=1?'within':'above'} earned-value cost expectations. Management attention should focus on delayed work fronts, approvals and cash collection.</p>`;}
  ...
  if(type==='claims'){rows=[['Reference','Event','Notice (BS)','Time Impact','Cost Impact','Status'],...p.claims.map(c=>[c.reference_id,c.title,formatBsDate(c.notice_date),c.time_impact_days,c.cost_impact_amount,c.status])];section=`<h2>Variation, Notice and EOT Register</h2>${table(rows)}`;}
  if(type==='quality'){rows=[['Inspection / Test','Date (BS)','Status','NCR','Open Days','Result'],...p.qaqc.map(q=>[q.qa_item,formatBsDate(q.inspection_date),q.status,q.ncr_number,q.ncr_open_days,q.test_result_details])];section=`<h2>QA/QC Dossier Index</h2>${table(rows)}`;}
  if(type==='safety'){rows=[['Date (BS)','Toolbox Talks','Incidents','Near Misses','Permits','Environmental Complaints'],...p.safety.map(s=>[formatBsDate(s.log_date),s.toolbox_talks,s.incidents,s.near_misses,s.permits_issued,s.environmental_complaints])];section=`<h2>EHS Performance</h2>${table(rows)}`;}
  if(type==='handover'){rows=[['Item','Category','Status','Approved By','Approved Date (BS)'],...p.handover.map(h=>[h.item_name,h.category,h.status,h.approved_by,formatBsDate(h.approved_date)])];section=`<h2>Handover and Completion Dossier</h2>${table(rows)}`;}
